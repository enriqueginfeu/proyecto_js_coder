const nombreForm = document.getElementById('nombre2')
const apellidoForm = document.getElementById('apellido2')
const emailForm = document.getElementById('email2')


const formulario = document.getElementById('form')

nombreForm.addEventListener('input', () => {
    const valor = nombreForm.value

    if (valor.length < 3) {
        nombreForm.classList.add('invalid')
        nombreForm.classList.remove('valid')
    } else {
        nombreForm.classList.add('valid')
        nombreForm.classList.remove('invalid')
    }
})

apellidoForm.addEventListener('input', () => {
    const valor = apellidoForm.value

    if (valor.length < 3) {
        apellidoForm.classList.add('invalid')
        apellidoForm.classList.remove('valid')
    } else {
        apellidoForm.classList.add('valid')
        apellidoForm.classList.remove('invalid')
    }

})

emailForm.addEventListener('input', () => {
    const valor = emailForm.value
...
nombreForm.classList.remove('invalid', 'valid');
apellidoForm.classList.remove('invalid', 'valid');
emailForm.classList.remove('invalid', 'valid')
form.reset()
